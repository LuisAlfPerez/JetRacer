...
while(True):
	...
	threshGaussian = cv.adaptiveThreshold(blurred,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY,11,2)
	openingMean = cv.morphologyEx(threshMean, cv.MORPH_OPEN, kernel)
	openingGaussian = cv.morphologyEx(threshGaussian, cv.MORPH_OPEN, kernel)
